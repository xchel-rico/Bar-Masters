...
class TestRateBarUseCase:
    ...
    def test_rate_bar_success(self, mocks):
        ...
        # Stubs
        bar_repo.get_by_id.return_value = Bar(
            10, "Bar", "End", "Desc", 1, "date")
        user_repo.get_by_id.return_value = User(
            5, "User", "email", "hash", "date")
        rating_repo.save.side_effect = lambda r: Rating(
            100, r.bar_id, r.user_id, r.score, r.comment, r.created_at)

        use_case = RateBarUseCase(bar_repo, user_repo, rating_repo)

        # Act
        result = use_case.execute(
            bar_id=10, user_id=5, score=5, comment="Excelente!")
